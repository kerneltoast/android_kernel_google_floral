				...
				struct device_attribute *attr, char *buf)
{
	...
	if (numberParameters >= 1) {
		...
		case 0x13:
			...
			if (numberParameters >= 2 && typeOfComand[1] == LOCKED_LP_ACTIVE)
		...
		case 0x15:
			...
			if (numberParameters >= 2 && typeOfComand[1] == LOCKED_LP_DETECT)
...
static int fts_probe(struct spi_device *client)
{
	...
	if (dp) {
		info->board = devm_kzalloc(&client->dev, sizeof(struct
								fts_hw_platform_data),
					   GFP_KERNEL);
		if (!info->board) {
